...
export function getUserPreferredLocale(shorten = false) {
  // Can be obtained with "window.navigator.languages" rather than "getUserLocale()".
  // let locale = getUserLocale()[0] ?? 'en'
...
/**
  *  Transforms { "en": "In pursuit of the dream." } into a "LocalizedTextObj" by getting the language name.
...
export function getLocalizedText(item: object): LocalizedText {
  ...
  return LocalizedTextObj(languageMetadata, (item as any)[locale])
}
...
export function getLocalizedTexts(item: object): LocalizedText[] {
  ...
  Object.keys(item).forEach(locale => {
    let languageMetadata = getLanguageMetadata(locale)
    let localizedText = LocalizedTextObj(languageMetadata, (item as any)[locale])
