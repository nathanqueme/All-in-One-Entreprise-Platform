...
// @ts-check
import { CategoryType, ImageType, ProfileButtonType, AccountType, TemporaryTime, TimetablesType, SpecialTimeType } from "./Types"
 ...
 */
export function PageObj(username: string, account_id: string, short_id: string) {
  return {
    username: username,
...
// NEW (web version only)
export interface FileImporterOutput {
  size: number
  uri: string
}
export function FileImporterOutputObj(size: number, uri: string) {
  return {
    size: size,
    uri: uri,
  }
