 ...
 * @param user_one indicates if the resource belongs to the user, so if the user is looking at it's information.
 */
...
export type ScreenNameType = 'scanner' | 'sign_in' | 'sign_up' | 'posts' | 'related_items' | 'profile' | 'qr' | 'pdf_viewer' | 'pdf_info' | 'settings' | 'account_info' | 'post_editor' | 'post_editor' | 'related_items_editor' | 'post_categories_sorter' | 'analytics'
...
export interface Session {
  ...
  version: string
  ip: string // UDPATED (was "ip_address")
  // APP ONLY 
  os_version?: string,
