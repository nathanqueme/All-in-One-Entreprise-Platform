...
// Localized text related
export async function generateSimplifiedTextLocalization(textLocalization: LocalizedText[]) {
    return new Promise(async (resolve, reject) => {

        let finalDescriptionLocalization = {}
        textLocalization.forEach(e => {
            finalDescriptionLocalization[e.language_metadata.locale] = e.text
        })

        resolve(finalDescriptionLocalization)
...
export async function generateSimplifiedLocalizedText(descriptionWasChanged: boolean, localizedText: LocalizedText) {
    return new Promise(async (resolve, reject) => {
        ...
        finalDescrtiption[languageLocale] = localizedText.text.trim()
...
export function allPhoneNumberWasChangedChecker(phoneNumber?: PhoneNumber, originalPhoneNumber?: PhoneNumber) {
...
export function timetablesWereChangedChecker(timetables: Timetables, originalTimetables: Timetables) {
    if (timetables === undefined && originalTimetables === undefined) return false
 ...
 */
export function handleTextAndPreventEmojis(text: string, setText: (_: string) => any, dispatch) {
