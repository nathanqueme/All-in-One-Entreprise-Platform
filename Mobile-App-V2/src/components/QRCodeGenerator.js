...
class QRCodeGenerator extends Component {
  ...
  // N.B.: adding = and => between (canvas) enables reading the this.state.username otherwise it crashes
  generateQRCode = (canvas) => {
    ...
    // QRCode options
    var options = {
      text: `https://www.atsight.ch/${this.state.username}?s_id=${this.props.short_id}&s=qr/`,
      width: this.state.width,
      height: this.state.height,
      PI: '#22baf7', // COLORS.atSightBlue, // Small cubes on the 3 corners
      colorDark: "#4e607b", // Center 
      colorLight: "white", // Background color : white
      correctLevel: QRCode.CorrectLevel.H, // L, M, Q, H


      AI: '#22baf7', // COLORS.atSightBlue
      ...
      logoWidth: this.state.width / 2.4,
      logoHeight: this.state.width / 2.4,
      logoBackgroundTransparent: true,

      onRenderingEnd: function (qrCodeOptions, base64DataFn) {
        base64DataFn().then(base64DataURL => {
          self.props.setQRCodeBase64(base64DataURL)
        })
      }
    }
